...
### ちゃんと同期できているか確認（おすすめ）
1. ログイン後、アプリ右上に **「✓ 同期済み」** と出ていればOK（「⚠ 同期エラー」が出たら、その文言をメモして A-3／A-4 を見直す）。
2. タスクを1つ追加 → Supabaseの **Table Editor → app_state** に行ができ、`data` の中にそのタスク名が入っていれば成功です。
3. 別端末で同じメール・パスワードでログイン → そのタスクが出てくれば同期成功。

> 安全設計：中身のあるデータが空データに上書きされないようになっています。空の端末でログインしても、既存データが消えることはありません。
